...
static const char *modnames[NUM_MODS] = {
	[MOD_CORE] = "core",
	[MOD_COMMON] = "common",
	[MOD_DRIVER] = "driver",
	[MOD_SCRAPE] = "scrape",
	[MOD_SCREEN] = "screen",
	[MOD_DRV_NOOP] = "drv_noop",
	[MOD_TASK_LOGIN] = "task_login",
	[MOD_TASK_MORE] = "task_more",
	[MOD_TASK_PROMPT] = "task_prompt",
	[MOD_UNKNOWN] = "(??" "?)"
};
...
void
nhd_log_setlvl(int mod, int lvl)
{
	s_lvlarr[mod] = lvl;
}

void
nhd_log_setlvl_all(int lvl)
{
...
int
nhd_log_getlvl(int mod)
{
	return s_lvlarr[mod];
}

void
nhd_log_log(int mod, int lvl, int errn, const char *file, int line,
...
nhd_log_init(void)
{
	...
	char v[128];
	if (getenv_m("NHDRV_DEBUG", v, sizeof v) == 0 && v[0]) {
	...
	const char *vv = getenv("NHDRV_DEBUG_FANCY");
	if (vv && vv[0] != '0')
		nhd_log_setfancy(true);
	else
		nhd_log_setfancy(false);
