...
bool
screen_findstr(const char *str, size_t *x, size_t *y)
{
	for (size_t j = 0; j < s_scr->height; j++) {
		const char *f = strstr((const char *)s_scr->canvas[j], str);
		if (f) {
			if (x) *x = f - (const char *)s_scr->canvas[j];
			if (y) *y = j;
